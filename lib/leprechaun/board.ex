defmodule Leprechaun.Board do
  ...
  def exists?(board) do
    Registry.lookup(Leprechaun.Board.Registry, board) != []
  end
  ...
  defp check(cells, n, acc, x, y, inc_x, inc_y) do
    if cells[y][x] == n do
      ...
      if new_x >= 1 and new_x <= @board_x and new_y >= 1 and new_y <= @board_y do
        ...
        Logger.debug(
          "[check] (#{x},#{y}) + (#{inc_x},#{inc_y}) -- #{n} #{inspect(acc)} limit reached"
        )
      ...
      Logger.debug(
        "[check] (#{x},#{y}) + (#{inc_x},#{inc_y}) -- #{cells[y][x]} != #{n} #{inspect(acc)}"
      )
